...
function calculateTotalSalaries() {
     ...
     function findEmployeesBySpecialization() {
        ...
        if (jsEmployees.length > 0) {
          const resultHTML = jsEmployees.map((employee) =>
            `<p>${employee.id}: ${employee.name} - ${employee.department} - $${employee.salary}</p>`
          ).join('');
          document.getElementById('employeesDetails').innerHTML = resultHTML;
        } else {
          document.getElementById('employeesDetails').innerHTML = '<p>No employees with JavaScript specialization found.</p>';
        }
      }
